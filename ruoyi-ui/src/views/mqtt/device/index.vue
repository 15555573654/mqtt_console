<template>
  <div class="app-container">
    ...
    <!-- 投屏控制弹窗 -->
    <screencast-dialog
      ref="screencastDialog"
      v-model="screencastDialogVisible"
      :device-name="screencastDevice"
...
export default {
  ...
  methods: {
    ...
    handleMqttMessage(topic, payload) {
      try {
        ...
        if (topic.includes('webrtc/')) {
          console.log('>>> 这是WebRTC信令消息，转发给组件处理');
          // 转发给投屏组件处理
          if (this.$refs.screencastDialog) {
            const { type } = data;
            if (type === 'answer') {
              this.$refs.screencastDialog.handleAnswer(data.answer);
            } else if (type === 'ice-candidate' && (!data.from || data.from === 'device')) {
              this.$refs.screencastDialog.handleIceCandidate(data.candidate);
            }
          }
          return;
        }
    ...
    /** 投屏控制 */
    handleScreencast() {
      // 这个方法已废弃，现在使用 openScreencastDialog
      this.openScreencastDialog();
    },
    /** 打开投屏弹窗 */
